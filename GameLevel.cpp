...
void Level_3()
{
	...
	for (int y = WALL_BOTTOM - 1; y > 7; y--)
	{
		gotoXY(57, y);
		cout << (char)222;
		wallchallenge_2.push_back(pii(57, y));
	}
}
void Level_4()
{
	vector<pii>wallchallenge;
	vector<pii>wallchallenge_2;
	vector<pii>wallchallenge_3;
	vector<pii>wallchallenge_4;
	setTextColor(12);
	drawWall();
	drawScoreBoard();
	setTextColor(12);
	...
	for (int x = 63; x < WALL_RIGHT-7; x++) {
		gotoXY(x,11);
		cout << (char)223;
		wallchallenge_2.push_back(pii(x, 11));
	}
	for (int x = WALL_LEFT+7; x <25; x++) {
		gotoXY(x,13);
		cout << (char)223;
		wallchallenge_3.push_back(pii(x, 13));
	}
	for (int y = 17; y < WALL_BOTTOM ; y++) {
		gotoXY(35, y);
		cout << (char)222;
		wallchallenge_4.push_back(pii(35, y));
	}
}
void Level_5()
{
